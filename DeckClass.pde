    ...
    if (Deck.length < 52) {  
      for (int i=0; i<4; i++) { //4 suits                         
        for (int j=0; j<13; j++) { //13 cards per suit           
          CardClass temp = new CardClass(j+1, i+1); // adds a card using i and j to decide a card suite and number.
          Deck = (CardClass[]) append(Deck, temp);//append adds a new card to the array
