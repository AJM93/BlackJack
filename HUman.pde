class Human extends DeckClass implements Player {
  ...
  Human(){}
  Human(DeckClass Deck) {
    Deckk=Deck;
    Deckk.CreateDeck();
    println(Deck);
    println(Deckk);
    println(Deckk.Deck[11]+"HI");
  ...
  //Deckk.Deck until it gets to cards_dealt
  ...
  void updateValueOfHand(/* you can add an argument here */) {
    ...
    //loops through the Deckk.Deck of cards from the first position to the amount of cards that have been dealt
    //adds the value of the current card in the Deckk.Deck to the value of the players hand
    for (int i = 0; i<cards_dealt;i++) {

      handvalue+=Deckk.Deck[i].CardValue;
      valuehand=""+handvalue;
    }
...
//    
  ...
  void showHand() {
    ...
    for (int i = 0; i<cards_dealt;i++) {
      ...
      Deckk.Deck[i].DrawCard(i+(100*i)+20, 50,true);

     // println(Deckk.Deck[i].CardValue);
    }
   // println(cards_dealt);
  }    
  //sets value of hand and cards dealt to 0 and als re-shuffles the Deckk.Deck. (might later need to add anotehr varieble in here to 
