...
a07
Note that the change in the file name is so that the project will work with the newest version of the processing.org beta
...
Dealers ai seems to be working fine now. A very brief answer to how i did it, would be that i tried very hard to ensure, that the dealers hand score
was being updated before it was being checked, and that the correct hand socre was being checked.

Added images for the cards (by making a variable called image, and setting that to the cardsuite and name and then loading an image of image name
and displaying that instead of the old box etc..

Adjustments made to the soft 18 code to further improve dealer AI
...
In this versio i have been working on improving the encapsulation of the program.

I have attempted to do this, by taking in a deck at teh players creation, and taking in a human or player at the Dealers creation.

Although the classes still rely on each other to work, they do not call anything from outside itself (e.g Deck.Deck use to use a deck outside of the class
and it now uses a deck within the actual class)
